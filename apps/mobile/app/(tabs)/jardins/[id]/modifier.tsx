...
import { useGarden, useUpdateGarden } from '@/lib/queries/gardens'
...
/**
 * Formulaire monté une fois le jardin chargé : ses valeurs par défaut sont
 * ainsi justes dès le premier rendu, sans réinitialisation après coup.
 */
function GardenForm({ garden }: { garden: GardenWithStats }) {
  ...
  const {
    ...
    defaultValues: {
      name: garden.name,
      type: (garden.type as GardenType) ?? 'OUTDOOR',
      description: garden.description ?? '',
      surfaceM2: garden.surfaceM2 ? String(garden.surfaceM2) : '',
    },
  })
...
export default function ModifierJardinScreen() {
    ...
    <SafeAreaView className="flex-1 bg-sand">
      ...
      >
        <View className="flex-row items-center justify-between px-4 py-3">
          ...
          <Text className="font-poppins text-section text-forest">Modifier</Text>
          <View className="w-16" />
        </View>
        ...
        {garden.isPending ? (
          ...
          <ErrorState message={errorMessage(garden.error)} onRetry={() => void garden.refetch()} />
        ) : (
          <GardenForm garden={garden.data} />
        )}
      </KeyboardAvoidingView>
    </SafeAreaView>
  )
