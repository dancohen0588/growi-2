...
export default function IdentifierPage() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    if (file.size > MAX_FILE_BYTES) {
      setErrorMsg('Image trop volumineuse (maximum 4 Mo).')
      return
    }
    setErrorMsg(null)
    try {
    ...
    <div className="flex flex-col gap-6 max-w-2xl mx-auto w-full">
      <header className="flex flex-col gap-1">
        ...
        <p className="font-raleway text-sm text-forest/60">
          Photographiez une plante — l&apos;IA l&apos;identifie et vous livre sa fiche.
        </p>
      </header>
...
function ResultStep({ preview, result, onReset }: ResultStepProps) {
  const [isPending, startTransition] = useTransition()
  const [addedPlantId, setAddedPlantId] = useState<string | null>(null)
  const [addError, setAddError] = useState<string | null>(null)

  const handleAddToMyPlants = useCallback(() => {
    ...
    startTransition(async () => {
      ...
      if (res.success && res.plantId) {
        ...
        setAddError(res.error ?? 'Impossible d\'ajouter la plante.')
      }
    })
  }, [result])
